...
class TrainingProcessStage1():
    ...
    def train(self, epoch):
        ...
        total_encoding_loss, total_cell_loss, total_contrastive_loss = 0., 0., 0.
        ...
        for batch_idx in range(self.training_iters):
            ...
            total_contrastive_loss += contrastive_loss.data.item()

            progress_bar(batch_idx, self.training_iters,
                         'encoding_loss: %.3f, rna_loss: %.3f, contrastive_loss: %.3f ' % (
                             total_encoding_loss / (batch_idx + 1), total_cell_loss / (batch_idx + 1),
                             total_contrastive_loss / (batch_idx + 1)
                         ))
        # save checkpoint
        save_checkpoint({
            'epoch': epoch,
